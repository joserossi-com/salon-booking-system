...
import { createCalendarEvent, findExistingEvent, isValidSalonDay } from "@/lib/google-calendar";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 4. Idempotencia: check si ya existe un evento en GCal en ese slot exacto.
    //    Previene duplicados cuando el mismo booking se procesa más de una vez.
    const existingEventId = await findExistingEvent(calendarId, inicio.toISOString(), finIso);
    const googleEventId = existingEventId ?? await createCalendarEvent({
      calendarId,
      summary:       `${servicio.nombre} — ${cliente_nombre}`,
      description:   `Cliente: ${cliente_nombre}\nTeléfono: ${cliente_telefono}\nServicio: ${servicio.nombre}`,
      startIso:      inicio.toISOString(),
      endIso:        finIso,
      trabajadoraId: trabajadora_id,
    });
    if (existingEventId) {
      console.log(`[calendar/appointments] Evento existente reutilizado: ${existingEventId}`);
    }
    ...
    if (cliErr || !cliente) {
      // Rollback: el evento de Google Calendar ya se creó pero no podemos
      // asociarlo a un cliente. Cancelamos para no dejar evento huérfano.
      try {
        const { cancelCalendarEvent } = await import("@/lib/google-calendar");
        await cancelCalendarEvent(calendarId, googleEventId);
      } catch (rollbackErr) {
        console.error("[calendar/appointments client-rollback]", rollbackErr);
      }
      throw cliErr;
    }
