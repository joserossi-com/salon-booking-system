  ...
  <div className="flex gap-0.5" aria-label="5 estrellas">
    {[...Array(5)].map((_, i) => (
      <svg key={i} viewBox="0 0 20 20" className="w-3.5 h-3.5" aria-hidden="true">
        <path fill="#A67C2A" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
      </svg>
    ))}
  </div>
);
...
export default function Resenas() {
  return (
    <section id="resenas" className="py-16 md:py-24 px-4 md:px-6" style={{ backgroundColor: "#F5EFF0" }}>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-5">
            ...
            <div className="flex items-center gap-2 pb-1">
              ...
              <span className="font-body text-sm font-semibold" style={{ color: "#1A1A1A" }}>5.0</span>
              <span className="font-body text-xs" style={{ color: "#6b5c68" }}>Google</span>
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
          {testimonios.map((t, i) => (
            <motion.article
              ...
              className="bg-white rounded-none p-5 md:p-8"
              style={{ boxShadow: "0 2px 20px rgba(45,27,46,0.06)" }}
            >
              <div className="mb-5">
                <StarRow />
              </div>
              <blockquote className="font-body text-base leading-relaxed mb-6 italic" style={{ color: "#1A1A1A" }}>
                &ldquo;{t.texto}&rdquo;
              </blockquote>
              <div className="flex items-center gap-3">
                <div
                  className="w-9 h-9 rounded-full flex items-center justify-center font-heading text-lg shrink-0"
                  style={{ backgroundColor: "rgba(212,137,158,0.12)", color: "#D4899E", border: "1px solid rgba(212,137,158,0.25)" }}
                  aria-hidden="true"
                >
                  {t.inicial}
                </div>
                <div>
                  <p className="font-heading text-xl leading-none" style={{ color: "#1A1A1A", letterSpacing: "0.04em" }}>{t.nombre.toUpperCase()}</p>
                  <p className="font-body text-xs mt-0.5" style={{ color: "#A67C2A" }}>{t.servicio}</p>
                </div>
              </div>
            </motion.article>
          ))}
